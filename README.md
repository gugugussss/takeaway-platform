...
4. **访问地址**
   - 前端：http://localhost:8080（默认，可在vue.config.js调整）
   - 后端接口：http://localhost:8081（默认，可在application.yml调整）
   - 后台管理：http://localhost:8081/admin

## 开发规范
- 前端：遵循Vue官方编码规范，组件化开发，统一API请求封装
- 后端：遵循RESTful接口设计，分层架构（Controller/Service/Mapper），统一异常处理
