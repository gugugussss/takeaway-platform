...
import com.atguigu.elm.dao.BusinessDao;
import com.atguigu.elm.dao.impl.BusinessDaoImpl;
...
public class BusinessViewImpl implements BusinessView {
	...
	public void listBusinessAll() {
		...
		System.out.println("商家编号\t商家名称\t商家地址\t商家介绍\t商家图片\t点餐分类\t起送费\t配送费\t备注");
		for (Business b : list) {

			System.out.println(b.getBusinessId() + "\t" + b.getBusinessName() + "\t" + b.getBusinessAddress() + "\t"
					+ b.getBusinessExplain() + "\t" + b.getBusinessImg() + "\t" + b.getOrderTypeId()+ "\t" + b.getStarPrice() + "\t" + b.getDeliveryPrice()+"\t" + b.getRemarks());
		}
	}
	...
	public Business login() {
	    ...
	    while (!input.hasNextInt()) {
	        System.out.println("商家编号格式错误！请重新输入商家编号：");
	        input.next();
	    }
	...
	public void editBusiness(Integer businessId) {
        ...
        System.out.println("是否修改商家图片(y/n)：");
        while (true) {
            inputStr = input.next();
            if (inputStr.equals("y") || inputStr.equals("n")) {
                break;
        ...
        if (inputStr.equals("y")) {
            System.out.println("请输入新的商家图片：");
            business.setBusinessImg(input.next());
        }

        System.out.println("是否修改点餐分类(y/n)：");
        while (true) {
            inputStr = input.next();
            if (inputStr.equals("y") || inputStr.equals("n")) {
                break;
        ...
        if (inputStr.equals("y")) {
            System.out.println("请输入新的点餐分类：");
            business.setOrderTypeId(input.nextInt());
        }
        ...
        if (inputStr.equals("y")) {
            ...
            while (input.nextDouble()<0) {
    	        System.out.println("输入错误！请重新输入新的起送费：");
    	        input.nextDouble();
    	    }     
        ...
        if (inputStr.equals("y")) {
        	System.out.println("请输入新的配送费：");
        	 while (input.nextDouble()<0) {
        	        System.out.println("输入错误！请重新输入新的配送费：");
        	        input.nextDouble();
        	    }
